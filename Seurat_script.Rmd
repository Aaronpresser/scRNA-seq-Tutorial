...
---
### I have/will added an R tutorial to this GitHub repository - it will show you how to install and set up R and is relatively comprehensive in showing you how to use it, and after you work through it and understand it, you should be competent enough to follow the tutorial here and do a good scRNA-seq analysis
...
# This link will tell you the different functions you have available for preprocessing your data, when you might use each function, and some possible arguments you can set for each function (https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/what-is-cell-ranger)
# This link will give you some tutorials on how to use the different preprocessingn frunctions
# There's also an example script in this GitHub repository on how to use the cellranger ```counts``` function - it is similar to the example in the tutorial a couple links up but it may be helpful to see another example
# Here are tutorials on using the Mac/Linux "command line" in your terminal application on your computer, which you will have to use in order to do these preprocessing things (https://www.youtube.com/watch?v=YHFzr-akOas) - for the Windows "command line," look here (https://www.cs.princeton.edu/courses/archive/spr05/cos126/cmd-prompt.html). Also Google around for other pedagogical presentations of the "command line," or if there are things that you don't particular understand how to do (e.g., "linux command line tutorial")
...
# This is another R tutorial - it is more comprehensive, but will take you longer to go through, and also perhaps to refer back to in the future, but still a very useful resource (here)
# First, with the exception of your dataset having different names, etc. than the ones in this tutorial or on the reference pages, if you are having trouble running a particular line of code, make sure the syntax of a particular line aligns EXACTLY with the syntax used either in this tutorial here, or on the reference page (you can find how to use very Seurat function here: https://satijalab.org/seurat/reference/)
# If you are having an issue running a particular line of code, GOOGLE IT! Someone has likely had your same issue before. If you poke around enough, you will often find an answer on Biostars, some random message board, or ...
# Seurat's GitHub issue page has a number of questions from people just like you, who are doing things very similar to you. Many of them have been answered - definintely try to find an answer there. If you can't, feel free to ask your question there as well.
# If googling doesn't work, perhaps concurrently with posting on Biostars or Seurat's GitHub issue page or a message board, email the IT department at your institution and ask them for help. They are often incredibly helpful and will solve most of your programming (but not actual analysis!) problems before they will be will be answered elsewhere. Look up "[my institution] research computing" or "[my institution] hpcc office" or "[my institution] it office" etc.
```
...
```{r}
# The three lines below - for the CTRL dataframe, it creates a column named "groups" and fills it with "CTRLL" all the way down. The TREAT line also creates a column named "groups" for the TREAT dataframe, and fills it with "TREAT" all the way down. The third line merges the two dataframes together
CTRL[["groups"]] <- "CTRL"
TREAT[["groups"]] <- "TREAT"
...
# In the few lines below, we split our data by "groups" (i.e., whether any particular piece of data is from the CTRL group or the TREAT group) and then we normalize it, and find the 3500 most variable genes
split_seurat <- lapply(X=split_seurat, FUN=function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, method="vst", nfeatures=3500)
})

# In these two lines, we essentially make sure that we are aligning our CTRL data and our TREAT data so when we generate UMAP or PCA plots, the data are relatively well aligned. Also, by coordinating the cleaning process for the CTRL and TREAT groups (which is what we are doing here), our analysis will generally be more accurate and precise
...
# These lines create (but do not actually plot) plots for the seurat_integrated object - the first one is split into "CTRL" and "TREAT" groups, and the second is generated with the clusters labelled
...
# Rather than having to run all the above lines of code again to get this processed seurat_integrated object next time we open our computer, we can save the seurat_integrated object to our computers and just load it again (fully-processed - see the "readRDS" line below) next time we want to use or reference it
...
```{r}
# This line uses the seurat_integrated object to create a new file on your computer than can be input into a piece of software that will automatically label the cell types of different cell clustsers - the software is not entirely accurate so you likely shouldn't rely entirely on it
...
# Create a reference dataframe so the software has some idea of what different cell types should look like - then if you input unlabelled data into it, it can try to predict what cell types those unlabelled data best look like
...
```{r}
# If your predictions look reasonable, you can relabel the clusters in your seurat_integrated object to have those predicted names, instead of just numbers as they have by default
...
```{r}
# You can then see which genes are conserved between two clusters like this - you can do this for clusters that you manually select with CellSelector() or clusters that are generated by way of k-means clustering
...
```{r}
# You can then see which genes are differentially expressed in a cluster (relative to another cluster that you specify) like this - you can do this for clusters that you manually select with CellSelector() or clusters that are generated by way of k-means clustering
